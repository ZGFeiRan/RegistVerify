...
<title>名医平台(系统管理平台)</title> <#include "../common/header.ftl"/>
...
<script type="text/javascript">
	$(function(){
		...
		$("#loginInfoDisplay").typeahead({
			minLength:3,//至少输入几个字符的时候,开始查询匹配
		...
		}).change(function(){// 使用链式的写法
			var current = $(this).typeahead("getActive");// 获取当前选中的数据
			if(current){
